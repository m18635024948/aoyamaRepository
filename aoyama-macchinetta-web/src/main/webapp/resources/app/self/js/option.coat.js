...
function initOptionCoat() {
	jQuery('#coatModel').attr("oldCtModel",jQuery('#coatModel').val());
	jQuery('#coatModel').change(function(){
		var coatModel = jQuery(this).val();
		var coatOldModel = jQuery('#coatModel').attr("oldCtModel");
		if(coatModel != coatOldModel){
			jQuery("#coatFlag").val("0");
		}else{
			jQuery("#coatFlag").val("1");
		}
		jQuery('#coatModel').attr("oldCtModel",jQuery('#coatModel').val());
		var coatFlag = jQuery("#coatFlag").val();
		// 選択中のCOATモデル
