package co.jp.aoyama.macchinetta.app.order;

public class OptionCodeKeys {
	
	public static final String TWO_PANTS = "0009902";
	
	
	// JACKET
	// フロント釦数:段返り3つボタン
	public static final String JK_0000101 = "0000101";
	// ラペルデザイン:ノッチ
	public static final String JK_0000201 = "0000201";
	// グレード:ハーフ毛芯
	public static final String JK_0000301 = "0000301";
	
	public static final String JK_0000304 = "0000304";
	// 裏仕様:総裏
	public static final String JK_0000401 = "0000401";

	public static final String JK_0000405 = "0000405";
	// 台場:
	public static final String JK_0000501 = "0000501";

	public static final String JK_0000502 = "0000502";

	public static final String JK_0000503 = "0000503";
	// 拝絹地
	public static final String JK_0000601 = "0000601";

	// 襟裏（ヒゲ）
	public static final String JK_0000701 = "0000701";
	// 襟吊
	public static final String JK_0000801 = "0000801";
	// ラペル幅
	public static final String JK_0000901 = "0000901";
	// フラワーホール
	public static final String JK_0001001 = "0001001";
	// 胸ポケット
	public static final String JK_0001101 = "0001101";
	// 腰ポケット
	public static final String JK_0001201 = "0001201";

	public static final String JK_0001205 = "0001205";
	// チェンジポケット
	public static final String JK_0001301 = "0001301";
	// スランテッドポケット
	public static final String JK_0001401 = "0001401";
	// 忍びポケット
	public static final String JK_0001501 = "0001501";
	// 袖仕様
	public static final String JK_0001601 = "0001601";
	// マニカ
	public static final String JK_0001701 = "0001701";
	// 袖釦
	public static final String JK_0001801 = "0001801";
	// 袖釦数
	public static final String JK_SLEEVE_BTN_CNT4 = "4";
	// 袖口
	public static final String JK_0001901 = "0001901";
	// 内ポケット変更
	public static final String JK_0002001 = "0002001";
	// 下前胸内ポケット仕様
	public static final String JK_0002201 = "0002201";
	// ステッチ種類
	public static final String JK_0002301 = "0002301";

	public static final String JK_0002302 = "0002302";

	public static final String JK_0002304 = "0002304";

	// ステッチ箇所変更
	public static final String JK_0002401 = "0002401";
	// ダブルステッチ変更
	public static final String JK_0002601 = "0002601";
	// AMF色指定
	public static final String JK_0002801 = "0002801";
	
	public static final String JK_0002802 = "0002802";
	// ボタンホール色指定
	public static final String JK_0003101 = "0003101";
	
	public static final String JK_0003102 = "0003102";
	// ボタン付け糸指定
	public static final String JK_0003401 = "0003401";
	
	public static final String JK_0003402 = "0003402";
	// ベント
	public static final String JK_0003701 = "0003701";
	// 胴裏素材
	public static final String JK_1000100 = "1000100";
	// 袖裏素材
	public static final String JK_2000100 = "2000100";
	// 釦素材
	public static final String JK_3000800 = "3000800";
	// 形状記憶
	public static final String JK_0004401 = "0004401";

	// PANTS
	// タック:ノータック
	public static final String PT_0000101 = "0000101";
	// 膝裏:有り
	public static final String PT_0000201 = "0000201";
	// 膝裏素材:ポリエステル
	public static final String PT_0000301 = "0000301";
	// フロント仕様:ホック留め
	public static final String PT_0000401 = "0000401";
	// パンチェリーナ:無し（ファスナー）
	public static final String PT_0000501 = "0000501";
	// アジャスター仕様:無し
	public static final String PT_0000601 = "0000601";
	// ベルトループ:有り
	public static final String PT_0000701 = "0000701";
	// ピンループ:有り
	public static final String PT_0000901 = "0000901";
	// 脇ポケット:斜め
	public static final String PT_0001002 = "0001002";
	// 忍びポケット:無し
	public static final String PT_0001104 = "0001104";
	// コインポケット:無し
	public static final String PT_0001201 = "0001201";
	// フラップ付コインポケット:無し
	public static final String PT_0001301 = "0001301";
	// 上前ピスポケット:ボタン有
	public static final String PT_0001401 = "0001401";
	// 下前ピスポケット:ボタン有
	public static final String PT_0001501 = "0001501";
	// Vカット:有り
	public static final String PT_0001602 = "0001602";
	// 裾上げ:シングル
	public static final String PT_0001701 = "0001701";
	// ダブル幅:4
	public static final String PT_4 = "4";
	// ステッチ種類:AMFステッチ
	public static final String PT_0001904 = "0001904";
	// ステッチ種類:無し
	public static final String PT_0001903 = "0001903";
	// ステッチ種類:ミシンステッチ
	public static final String PT_0001901 = "0001901";
	// ステッチ箇所変更:無し
	public static final String PT_0002001 = "0002001";
	
	public static final String PT_0002002 = "0002002";
	// ダブルステッチ:無し
	public static final String PT_0002201 = "0002201";
	
	public static final String PT_0002202 = "0002202";
	// AMF色指定:無し
	public static final String PT_0002401 = "0002401";
	
	public static final String PT_0002402 = "0002402";
	// ボタンホール色指定:無し
	public static final String PT_0002701 = "0002701";
	
	public static final String PT_0002702 = "0002702";
	// ボタン付け糸指定:無し
	public static final String PT_0003001 = "0003001";
	
	public static final String PT_0003002 = "0003002";
	// 釦素材:ユリア
	public static final String PT_3000800 = "3000800";
	// サスペンダー釦:無し
	public static final String PT_0003501 = "0003501";
	// シック大（股補強）:無し
	public static final String PT_0004101 = "0004101";
	// エイト（滑り止め）:無し
	public static final String PT_0003601 = "0003601";
	// 形状記憶:無し
	public static final String PT_0003701 = "0003701";
	// 側章:有り
	public static final String PT_0003902 = "0003902";
	// 側章幅:1.5cm
	public static final String PT_0004001 = "0004001";

	// COAT
	// ラペルデザイン
	public static final String CT_0000101 = "0000101";
	// 腰ポケット
	public static final String CT_0000201 = "0000201";
	// チェンジポケット
	public static final String CT_0000301 = "0000301";
	// スランテッドポケット
	public static final String CT_0000401 = "0000401";
	// ベント
	public static final String CT_0000501 = "0000501";
	// フロント釦数
	public static final String CT_0000601 = "0000601";
	// 袖口
	public static final String CT_0000701 = "0000701";
	// 袖釦
	public static final String CT_0000801 = "0000801";
	// バックベルト
	public static final String CT_0000901 = "0000901";
	// 襟吊
	public static final String CT_0001001 = "0001001";
	// 胴裏素材
	public static final String CT_4000100 = "4000100";
	// 袖裏素材
	public static final String CT_5000100 = "5000100";
	// 釦素材
	public static final String CT_6000100 = "6000100";
	// 釦素材
	public static final String CT_0001401 = "0001401";

	// GILET
	// 胸ポケット
	public static final String GL_0000101 = "0000101";
	public static final String GL_0000102 = "0000102";
	public static final String GL_0000103 = "0000103";
	// 腰ポケット
	public static final String GL_0000201 = "0000201";
	// 腰ポケット形状
	public static final String GL_0000301 = "0000301";
	public static final String GL_0000303 = "0000303";
	// 腰ポケット素材
	public static final String GL_0000401 = "0000401";
	// ステッチ種類
	public static final String GL_0000504 = "0000504";
	// ステッチ箇所変更
	public static final String GL_0000601 = "0000601";
	// ダブルステッチ
	public static final String GL_0002601 = "0002601";
	// AMF色指定
	public static final String GL_0000801 = "0000801";
	// ボタンホール色指定
	public static final String GL_0001101 = "0001101";
	// ボタン付け糸指定
	public static final String GL_0001401 = "0001401";
	// 背裏地素材
	public static final String GL_1000100 = "1000100";
	// 内側裏地素材
	// public static final String GL_1000100 = "1000100";
	// 釦素材
	public static final String GL_3000800 = "3000800";
	// バックベルト
	public static final String GL_0002301 = "0002301";
	public static final String GL_0002302 = "0002302";
	// ウォッチチェーン
	public static final String GL_0002401 = "0002401";

	// SHIRT
	// 襟型
	public static final String sht_0000100 = "0000100";
	// カフス
	public static final String sht_0000200 = "0000200";
	// コンバーチブル
	public static final String sht_0000301 = "0000301";
	// アジャスト釦
	public static final String sht_0000401 = "0000401";
	// クレリック仕様
	public static final String sht_0000501 = "0000501";
	// ダブルカフス仕様
	public static final String sht_0000601 = "0000601";
	// カフスボタン追加
	public static final String sht_0000701 = "0000701";
	// 釦素材
	public static final String sht_0000801 = "0000801";
	// タブ釦
	public static final String sht_0000901 = "0000901";
	// ガントレットボタン位置
	public static final String sht_0001001 = "0001001";
	// ピンホールピン
	public static final String sht_0001101 = "0001101";
	// 胸ポケット
	public static final String sht_0001201 = "0001201";
	// 胸ポケット大きさ
	public static final String sht_0001301 = "0001301";
	// フロントデザイン
	public static final String sht_0001401 = "0001401";
	// ピンタックブザム
	public static final String sht_0001501 = "0001501";
	// ステッチ
	public static final String sht_0001601 = "0001601";
	// カラーキーパー
	public static final String sht_0001701 = "0001701";
	// ボタンホール色変更
	public static final String sht_0001901 = "0001901";
	// ボタン付け糸色変更
	public static final String sht_0001801 = "0001801";
	// カジュアルヘムライン仕様
	public static final String sht_0002001 = "0002001";
	// ボタン位置変更
	public static final String sht_0002101 = "0002101";

	// PANTS2
	// タック:ノータック
	public static final String PT2_0000101 = "0000101";
	// 膝裏:有り
	public static final String PT2_0000201 = "0000201";
	// 膝裏素材:ポリエステル
	public static final String PT2_0000301 = "0000301";
	// フロント仕様:ホック留め
	public static final String PT2_0000401 = "0000401";
	// パンチェリーナ:無し（ファスナー）
	public static final String PT2_0000501 = "0000501";
	// アジャスター仕様:無し
	public static final String PT2_0000601 = "0000601";
	// ベルトループ:有り
	public static final String PT2_0000701 = "0000701";
	// ピンループ:有り
	public static final String PT2_0000901 = "0000901";
	// 脇ポケット:斜め
	public static final String PT2_0001002 = "0001002";
	// 忍びポケット:無し
	public static final String PT2_0001104 = "0001104";
	// コインポケット:無し
	public static final String PT2_0001201 = "0001201";
	// フラップ付コインポケット:無し
	public static final String PT2_0001301 = "0001301";
	// 上前ピスポケット:ボタン有
	public static final String PT2_0001401 = "0001401";
	// 下前ピスポケット:ボタン有
	public static final String PT2_0001501 = "0001501";
	// Vカット:有り
	public static final String PT2_0001602 = "0001602";
	// 裾上げ:シングル
	public static final String PT2_0001701 = "0001701";
	// ダブル幅:4
	public static final String PT2_4 = "4";
	// ステッチ種類:AMFステッチ
	public static final String PT2_0001904 = "0001904";
	// ステッチ種類:無し
	public static final String PT2_0001903 = "0001903";
	// ステッチ種類:ミシンステッチ
	public static final String PT2_0001901 = "0001901";
	// ステッチ箇所変更:無し
	public static final String PT2_0002001 = "0002001";
	
	public static final String PT2_0002002 = "0002002";
	// ダブルステッチ:無し
	public static final String PT2_0002201 = "0002201";
	
	public static final String PT2_0002202 = "0002202";
	// AMF色指定:無し
	public static final String PT2_0002401 = "0002401";
	
	public static final String PT2_0002402 = "0002402";
	// ボタンホール色指定:無し
	public static final String PT2_0002701 = "0002701";
	
	public static final String PT2_0002702 = "0002702";
	// ボタン付け糸指定:無し
	public static final String PT2_0003001 = "0003001";
	
	public static final String PT2_0003002 = "0003002";
	// 釦素材:ユリア
	public static final String PT2_3000800 = "3000800";
	// サスペンダー釦:無し
	public static final String PT2_0003501 = "0003501";
	// シック大（股補強）:無し
	public static final String PT2_0004101 = "0004101";
	// エイト（滑り止め）:無し
	public static final String PT2_0003601 = "0003601";
	// 形状記憶:無し
	public static final String PT2_0003701 = "0003701";
	// 側章:有り
	public static final String PT2_0003902 = "0003902";
	// 側章幅:1.5cm
	public static final String PT2_0004001 = "0004001";
}
